...
int uncompress(void *dest, unsigned int *destLen,
  ...
  for (unsigned int i = 0; i < data->count; i++)
  {
    if (strcmp(data->files[i].name, filename) == 0)
    {
      ...
      if (f.compression > 0)
      {
        ...
        if (f.compression == 1)
          uncompress((unsigned char*)buffer2, (unsigned int*)&realsize, (unsigned char*)&buffer[8], size + 8);
